...
mkdir -p $PROJECTDIR/bed/correlations_nodups
...
if [[ $SAMPLE_NAME == *_CM_* ]]; then
	...
	bedtools bamtobed -i $PROJECTDIR/mapped/$SAMPLE_NAME.trimmed.bowtie2.sorted.shifted.nodups.bam | \
	bedtools intersect -c -a $WINDOWS -b stdin | \
	bedtools sort \
	> $PROJECTDIR/bed/correlations_nodups/${SAMPLE_NAME}_1kb_windows.bed
	
	cut -f 4 $PROJECTDIR/bed/correlations_nodups/${SAMPLE_NAME}_1kb_windows.bed | \
	sed "1s/^/$SAMPLE_NAME\n/" \
	> $PROJECTDIR/bed/correlations_nodups/${SAMPLE_NAME}_1kb_windows_1col.bed

elif [[ $SAMPLE_NAME == *_ChIP_* ]]; then
	echo "Counting reads on ChIP sample: " $SAMPLE_NAME
	bedtools bamtobed -i $PROJECTDIR/mapped/$SAMPLE_NAME.trimmed.bowtie2.sorted.nodups.bam | \
	bedtools intersect -c -a $WINDOWS -b stdin | \
	bedtools sort \
	> $PROJECTDIR/bed/correlations_nodups/${SAMPLE_NAME}_1kb_windows.bed
