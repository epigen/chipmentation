...
p = ggplot(df, aes(sample, postProb)) +
    geom_boxplot() +
    theme_bw()
ggsave(filename = paste0(plotsDir, sample, "_peaks.footprintsCentipede.400bp.PU1.boxplot.pdf"),
    plot = p, height = 4, width = 2.5)

p = ggplot(df, aes(sample, postProb)) +
    geom_violin(alpha=0.5, color="gray") + 
    theme_bw()
ggsave(filename = paste0(plotsDir, sample, "_peaks.footprintsCentipede.400bp.PU1.violinplot.pdf"),
    plot = p, height = 4, width = 2.5)
...
gata = read.csv(paste0(bedDir, "PU1_K562_10mio_CM_peak_coverage.GATAmotif.csv"))
rownames(gata) = gata[,1]
colnames(gata) = seq(-2000, 1999)
gata = gata[,-1]
...
clusterOrder = clusterOrder[, c("X", "NAME", "GWEIGHT", paste("X", 1:400, sep = ""))]
clusterOrder = as.data.frame(rbind(c("EWEIGHT", "", "", rep(1, 400)), clusterOrder))
write.table(clusterOrder, paste0(resultsDir, sample, ".GATAmotif.heatmap.cdt"), sep = '\t', quote = FALSE, row.names = FALSE, col.names = TRUE) # all


# make PostProb plots
df = data.frame(postProb = centFitGATA$PostPr, sample = paste0("GATA motifs"))

p = ggplot(df, aes(sample, postProb)) +
    geom_boxplot() +
    theme_bw()
ggsave(filename = paste0(plotsDir, sample, "_peaks.GATAmotif.footprintsCentipede.400bp.PU1.boxplot.pdf"),
    plot = p, height = 4, width = 2.5)

p = ggplot(df, aes(sample, postProb)) +
    geom_violin(alpha=0.5, color="gray") + 
    theme_bw()
ggsave(filename = paste0(plotsDir, sample, "_peaks.GATAmotif.footprintsCentipede.400bp.PU1.violinplot.pdf"),
    plot = p, height = 4, width = 2.5)
