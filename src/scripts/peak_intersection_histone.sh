...
#bedtools multiinter -i $PROJECTDIR/peaks/${SAMPLE_CM500k}_peaks/${SAMPLE_CM500k}_peaks.narrowPeak  \
#$PROJECTDIR/peaks/${SAMPLE_CM10k}_peaks/${SAMPLE_CM10k}_peaks.narrowPeak \
#$PROJECTDIR/peaks/${SAMPLE_CHIP}_peaks/${SAMPLE_CHIP}_peaks.narrowPeak \
#$PROJECTDIR/peaks/${SAMPLE_ENCODE}_peaks/${SAMPLE_ENCODE}_peaks.narrowPeak \
#> inter
...
for SAMPLE_NAME in H3K4me3_K562_500k_CM
    ...
    SAMPLE_CM500k_COUNT="$(wc -l < $PROJECTDIR/peaks/${SAMPLE_CM500k}_peaks/${SAMPLE_CM500k}_peaks.narrowPeak)"
    echo "Counting peaks in" $SAMPLE_CM10k
    SAMPLE_CM10k_COUNT="$(wc -l < $PROJECTDIR/peaks/${SAMPLE_CM10k}_peaks/${SAMPLE_CM10k}_peaks.narrowPeak)"
    echo "Counting peaks in" $SAMPLE_CHIP
    SAMPLE_CHIP_COUNT="$(wc -l < $PROJECTDIR/peaks/${SAMPLE_CHIP}_peaks/${SAMPLE_CHIP}_peaks.narrowPeak)"
    echo "Counting peaks in" $SAMPLE_ENCODE
    SAMPLE_ENC_COUNT="$(wc -l < $PROJECTDIR/peaks/${SAMPLE_ENCODE}_peaks/${SAMPLE_ENCODE}_peaks.narrowPeak)"
