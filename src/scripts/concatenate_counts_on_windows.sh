...
while read SAMPLE_NAME CONTROL_NAME; do
    echo "Doing sample: " $SAMPLE_NAME "and control: " $CONTROL_NAME
    cut -f 4 $PROJECTDIR/bed/correlations/${SAMPLE_NAME}_1kb_windows.bed | sed "1s/^/$SAMPLE_NAME\n/" > $PROJECTDIR/bed/correlations/${SAMPLE_NAME}_1kb_windows_1col.bed
    cut -f 4 $PROJECTDIR/bed/correlations/${CONTROL_NAME}_1kb_windows.bed | sed "1s/^/$CONTROL_NAME\n/" > $PROJECTDIR/bed/correlations/${CONTROL_NAME}_1kb_windows_1col.bed
    if [[ $COUNTER == 0 ]]
        then
        cp $PROJECTDIR/bed/correlations/${SAMPLE_NAME}_1kb_windows_1col.bed $COUNTS
        paste $COUNTS $PROJECTDIR/bed/correlations/${CONTROL_NAME}_1kb_windows_1col.bed > tmp
        mv tmp $COUNTS
    else
